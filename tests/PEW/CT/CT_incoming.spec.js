...
test('test @FP-TC-328', async ({ page }) => {
  ...
  const creditPortfolioInput = page.locator('#creditPortfolio2 .ac-input-wrapper > .form-control');
  await creditPortfolioInput.click();
  await creditPortfolioInput.fill('sta');
  await creditPortfolioInput.press('Tab');
  await page.getByRole('row', { name: 'IRON MAN 1010A-01 TONY' }).press('Enter');
  await page.locator('#debitPortfolio').click();
